...
#if not downloaded, download dataset and unzip
!if [ ! -f "games.csv"] || [ ! -f "games.json" ]; then kaggle datasets download -d fronkongames/steam-games-dataset; fi

#unzip if not unzipped
!if [ ! -f "steam-games-dataset.zip" ]; then unzip steam-games-dataset.zip; fi
...
# SAMPLE PROVIDED WITH KAGGLE DATASET
...
"""# !!*Currently Here Working on One-Hot encoding*!!"""
...
for index, game in game_data.iterrows():
  genre_list.extend(game['genres'])
  category_list.extend(game['categories'])

unique_genres = list(set(genre_list))
unique_categories = list(set(category_list))\
...
game_data['genres'].info()
game_data['categories'].info()

for genre in game_data['genres']:
  print(genre)

# Step 4: Prepare the collaborative filtering matrix (user-game interaction)
user_game_interactions = pd.DataFrame(user_game_interactions, columns=['user_id', 'game_id', 'interaction'])
user_game_interactions = user_game_interactions.pivot_table(index='user_id', columns='game_id', values='interaction', aggfunc='mean')
